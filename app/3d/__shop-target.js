...
class ShopTarget extends Target {
	...
	setupItems() {
		const step = window.mobile ? 1.4 : 1.33;
		const axis = new THREE.Vector3(0, 0, 1);
		const position = new THREE.Vector3((window.mobile ? 20 : 10), 0, 0);
	...
	activate(activate) {
		...
		if (this.isActivated) {
			this.shopItems.forEach(item => item.show());
			TweenLite.to(this.target.children[0].rotation, 0.77, {
				z: Math.PI * 2,
