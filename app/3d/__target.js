...
import { TARGET_RADIUS, TARGET_HITAREA_RADIUS, TARGET_FOCUS_SCALE, SCALE_SPRING, SCALE_DAMPING, TARGET_MAX_WANDER, BUBBLE_SRC, NOISE_OFFSET_STEP, NOISE_OFFSET_MAX } from './constants.js';
...
class Target extends THREE.Object3D {
	...
	setupTarget() {
		...
		const material = new THREE.MeshStandardMaterial({
			metalness: 0,
			roughness: 0,
			map: textureLoader.load(this.BUBBLE_SRC),
			transparent: true,
			depthTest: false,
		});
	...
	update(delta) {
		this.noiseTime += NOISE_OFFSET_STEP * delta;
	...
	updatePosition(delta) {
		...
		this.noiseOffset.set(
			this.noise.simplex2(this.noiseTime, 0) * NOISE_OFFSET_MAX,
			this.noise.simplex2(this.noiseTime, 100) * NOISE_OFFSET_MAX,
			this.noise.simplex2(this.noiseTime, 200) * NOISE_OFFSET_MAX,
		);
		...
		this.position.add(this.noiseOffset);
	}
}
